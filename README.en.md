...
- Unsure about the machine: start with `gpu-holder doctor --backend driver`.
- Do not want PyTorch: try `--backend driver` first, then follow doctor hints if driver/device
  access fails.
- Already have CUDA-enabled PyTorch: the default `--backend torch` is fine.
- First run on a shared machine: use `--dry-run --explain` to inspect every per-GPU decision.
- Ready to hold in foreground: remove `--dry-run`; for background mode use `gpu-holder start ...`.

`gpu-holder` is a small NVIDIA GPU guard for shared training machines. It starts lightweight CUDA
holder workers when selected GPUs fall below a per-GPU utilization floor, and it yields to real
training or inference jobs without killing, suspending, renicing, or modifying external processes.
...
Shared GPU machines are often reclaimed or flagged as idle when utilization drops during data
loading, startup, checkpointing, or stalled training. `gpu-holder` keeps utilization above a
configured floor while preserving a narrow safety boundary:
...
All utilization and memory ratios use `0..1` floats. Legacy percent forms like `60`, `60%`, and
`0.6` are accepted and normalized to `0.6`.
...
- `--busy-process-mem-threshold 10GiB`: classify large external CUDA processes as training or
  inference jobs.
- `--process-grace-window 120`: let large external processes initialize for 120 seconds before
  assist is allowed.
...
5. Grace window has expired and utilization is still below `risk-util`: start an assist holder with
   `--assist-mem`.
...
Base install has no third-party runtime dependency. It uses system `nvidia-smi` for read-only GPU
status.
...
This should be described as "works on machines with NVIDIA driver and accessible CUDA devices", not
